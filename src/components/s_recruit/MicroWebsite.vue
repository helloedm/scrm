...
<div class="MicroWebsite">
  <div id="MicroWebsite">
    ...
    <div class="submit_btn" v-show="is_load">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getCompanyDetail(){
        var _this = this;
        var method = "companyWeb/getCompanyDetailForPC";
        var param = JSON.stringify({
          id: this.$route.query.id
        });
        var successd = function (res) {
      ...
      this.box.$el.addEventListener('scroll', () => {
        ...
        for(let i = 0; i < loitiArr.length; i++){
          ...
          if(loitiArr[i].offsetTop <= contentScrollTop){
            ...
            if(contentheight==contentScrollTop+575){ //滑动到了底部
              this.active = 5
            }
          }
        }
...
<style scoped>
.MicroWebsite{max-width:1066px;margin:0 auto;background-color: #fff;padding-left:20px;height: 575px;}
