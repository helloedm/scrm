<template>
  <dl class="hrm_module" id="newRe">
    <div class="btn_top">
      ...
      <el-button type="text" @click="channelSet()" style="margin-left:30px;"><i class="iconfont hrm_btn_icon">&#xe65f;</i>渠道导入</el-button>
      <el-button type="text" @click="emailToLead()" style="margin-left:30px;"><i class="iconfont hrm_btn_icon">&#xe660;</i>邮箱导入简历</el-button>
    </div>
    ...
    <dd class="hrm_module_con">
      ...
      <el-table-column
        ...
        <template scope="scope">
          ...
          <el-button type="text" @click="resume(scope.row.id)" :disabled="scope.row.id==-1">{{scope.row.name}}</el-button>
        </template>
      </el-table-column>
      <el-table-column
        prop="phone"
...
export default {
  ...
  mounted(){
    ...
    }else if(this.$route.query.processStatus){
      this.wait_screened();
    }else{
  ...
  methods:{
    wait_screened(){
      ...
      var method="queryResume/getPositionRedirectionPage",
    ...
    resume(id){
      if(id==-1){
        this.$message.error('没有简历详情,请联系客服~~')
      }else{
